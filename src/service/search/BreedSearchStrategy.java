...
public class BreedSearchStrategy implements PetSearchStrategy {
    @Override
    ...
    public List<Path> search(PetType petType, PetCollector collector, FileRepository fileRepository) {
        ...
        String petBreed = collector.collectPetBreed();
        List<Path> allPets = fileRepository.listAllPaths();
        List<Path> filteredPets = new ArrayList<>();
        Pattern agePattern = Pattern.compile("\\b" + petBreed + "\\b", Pattern.CASE_INSENSITIVE);
        for (Path s : allPets) {
